...
function twentysixteen_custom_header_and_background() {
	 ...
	 *     @type int      $height           Height in pixels of the custom header image. Default 610.
...
function twentysixteen_get_color_schemes() {
	 ...
	 *                              background, link, main text, secondary text, header background color.
	 *     }
	 * }
	 */
	return apply_filters( 'twentysixteen_color_schemes', array(
		'europa' => array(
			'label'  => __( 'Europa', 'twentysixteen' ),
			'colors' => array(
				'#1a1a1a',
				'#ffffff',
				'#055899',
...
function twentysixteen_color_scheme_css() {
	...
	// Convert header background hex color to rgba.
	$header_bg_color_rgb = twentysixteen_hex2rgb( $color_scheme[5] );

	// If the rgba values are empty return early.
	if ( empty( $header_bg_color_rgb ) ) {
		return;
	}
	...
	$colors = array(
		...
		'header_background_color' 	=> $color_scheme[5],
		'header_text_color'		 	=> $color_scheme[6],
		'border_color'          	=> vsprintf( 'rgba( %1$s, %2$s, %3$s, 0.2)', $color_textcolor_rgb ),
		'header_bg_color_rgb'       => vsprintf( 'rgba( %1$s, %2$s, %3$s, 0.8)', $header_bg_color_rgb ),
...
function twentysixteen_get_color_scheme_css( $colors ) {
	...
	// Check to see if there's a header image set 
	if ( get_header_image() ) {
		// Use Header RGB with Image
		$header_background_color = 'linear-gradient(' . $colors['header_bg_color_rgb'] . ',' . $colors['header_bg_color_rgb'] .  '),url(' . get_header_image() . ') top center/cover';
	}
	else {
...
function twentysixteen_color_scheme_css_template() {
	$colors = array(
		...
		'header_bg_color_rgb'          => '{{ data.header_bg_color_rgb }}',
	);
	?>
...
function twentysixteen_header_background_color_css() {
	...
	if ( get_header_image() ) {
		// Convert main text hex color to rgba.
		$header_bg_color_rgb = twentysixteen_hex2rgb( $header_background_color );
		$header_bg_color_rgb = vsprintf( 'rgba(%1$s, %2$s, %3$s, 0.8)', $header_bg_color_rgb );
		$header_bg_color_rgb = 'linear-gradient(' . $header_bg_color_rgb . ',' . $header_bg_color_rgb .  '),url(' . get_header_image() . ') top center/cover';
		$css = '
	...
	wp_add_inline_style( 'twentysixteen-style', sprintf( $css, $header_background_color, $header_bg_color_rgb ) );
}
